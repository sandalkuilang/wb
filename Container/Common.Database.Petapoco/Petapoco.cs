...
namespace PetaPoco
{
    ...
    public class Database : IDisposable
    {
        ...
        public IDbCommand CreateCommand(IDbConnection connection, string sql, params object[] args)
        {
            ...
            if (_dbType == DBType.Oracle)
            {
                PropertyInfo bindByName = cmd.GetType().GetProperty("BindByName");
                if (bindByName != null)
        ...
        public IEnumerable<T> Query<T>(string sql, params object[] args)
        {
            ...
            { 
                using (var cmd = CreateCommand(_sharedConnection, sql, args))
                {
                    ...
                    try
                    {
                        r = cmd.ExecuteReader(CommandBehavior.SequentialAccess);
                        OnExecutedCommand(cmd);
                    }
                    catch (Exception x)
                    {
        ...
        public class PocoData
        {
            ...
            public static PocoData ForType(Type t)
            {
                ...
                try
                {
                    if (m_PocoDatas.Contains(t.Name))
                        pd = (PocoData)m_PocoDatas.Get(string.Join(".", t.Name, t.GetHashCode().ToString()));
                ...
                try
                {
                    ...
                    m_PocoDatas.Add(string.Join(".", t.Name, t.GetHashCode().ToString()), pd, policy);
                }
                finally
                {
                    RWLock.ExitWriteLock();
            ...
            static ObjectCache m_PocoDatas = new MemoryCache("petaPoco");
